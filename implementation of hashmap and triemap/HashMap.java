...
public class HashMap<K, V> extends BaseAbstractMap<K, V> {
    ...
    public V get(Object key) {
        ...
        if (key == null) {
            ...
            hash = hash(key.hashCode(), table.length); 
        }
    ...
    public V put(K key, V value) {
        ...
        Entry<K, V> newEntry = new Entry<K, V>(key, value, table[hash]); 
        table[hash] = newEntry; 
        size++; 
        if (size >= threshold) {
