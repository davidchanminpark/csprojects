...
public class SwapCorrector extends Corrector {
    ...
    public Set<String> getCorrections(String wrong) {
        ...
        for (int i = 0; i < wrong.length() - 1; i++) {
            String before = wrong.substring(0, i);
            String after = wrong.substring(i + 2);
            char x = wrong.charAt(i);
            char y = wrong.charAt(i + 1);
            String s = before + y + x + after;
            
            if (d.isWord(s)) {
                recs.add(s);
            }
        }
        return matchCase(wrong, recs);
    }
}
