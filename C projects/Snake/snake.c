...
void turn_snake(int new_direction) {
    if (new_direction - snake_direction < 2 || new_direction - snake_direction > 2) {
...
void spawn_fruit() {
    ...
    lc4uint checkNew = 1; 
    while (checkNew == 1) { 
        fruitx = rand16() % COLUMNS; 
        fruity = rand16() % ROWS;
        checkSame = 0; 
        while (checkSame == 0) {
            for (i = 0; i < MAX_BOMBS; i++) {
                if (fruitx == bomb[i].x && fruity == bomb[i].y) {
                    checkSame = 1; 
                }
            }
            /* If there is no same one, then exit out of loop */
            checkNew = 0; 
        }
        /* If there is a same one then go back to top loop */
    }
    fruit.x = fruitx; 
    fruit.y = fruity; 
}
...
void spawn_bomb() {
    ...
    lc4uint checkNew = 1; 
    while (checkNew == 1) { 
        bombx = rand16() % COLUMNS; 
        bomby = rand16() % ROWS;
        checkSame = 0; 
        while (checkSame == 0) {
            if (bombx == fruit.x && bomby == fruit.y) {
                checkSame = 1; 
            }
            for (i = 0; i < MAX_BOMBS; i++) {
                if (bombx == bomb[i].x && bomby == bomb[i].y) {
                    checkSame = 1; 
                }
            }
            /* If there is no same one, then exit out of loop */
    ...
    bomb[bombs_count].x = bombx; 
    bomb[bombs_count].y = bomby;  
    bombs_count++; 
}
...
int update_game_state() {
    ...
    if (collision == BOMB || collision == SNAKE) {
        return GAME_LOST; 
    }
    if (collision == FRUIT) {
        spawn_fruit(); 
...
void display_score() {
    ...
    if (snake_length == 3) {
        lc4_draw_sprite(0, 0, YELLOW, three);
    }
    if (snake_length == 4) {
        lc4_draw_sprite(0, 0, YELLOW, four);
    }
    if (snake_length == 5) {
        lc4_draw_sprite(0, 0, YELLOW, five);
    }
    if (snake_length == 6) {
        lc4_draw_sprite(0, 0, YELLOW, six);
    }
    if (snake_length == 7) {
        lc4_draw_sprite(0, 0, YELLOW, seven);
    }
    if (snake_length == 8) {
        lc4_draw_sprite(0, 0, YELLOW, eight);
    }
    if (snake_length == 9) {
        lc4_draw_sprite(0, 0, YELLOW, nine);
    }
    if (snake_length == 10) {
        lc4_draw_sprite(0, 0, YELLOW, one);
        lc4_draw_sprite(8, 0, YELLOW, zero);
    }
    if (snake_length == 11) {
        lc4_draw_sprite(0, 0, YELLOW, one);
        lc4_draw_sprite(8, 0, YELLOW, one);
    }
    if (snake_length == 12) {
        lc4_draw_sprite(0, 0, YELLOW, one);
        lc4_draw_sprite(8, 0, YELLOW, two);
    }
    if (snake_length == 13) {
        lc4_draw_sprite(0, 0, YELLOW, one);
        lc4_draw_sprite(8, 0, YELLOW, three);
    }
    if (snake_length == 14) {
        lc4_draw_sprite(0, 0, YELLOW, one);
        lc4_draw_sprite(8, 0, YELLOW, four);
    }
    if (snake_length == 15) {
        lc4_draw_sprite(0, 0, YELLOW, one);
        lc4_draw_sprite(8, 0, YELLOW, five);
    }
    if (snake_length == 16) {
        lc4_draw_sprite(0, 0, YELLOW, one);
        lc4_draw_sprite(8, 0, YELLOW, six);
    }
    if (snake_length == 17) {
        lc4_draw_sprite(0, 0, YELLOW, one);
        lc4_draw_sprite(8, 0, YELLOW, seven);
    }
    if (snake_length == 18) {
        lc4_draw_sprite(0, 0, YELLOW, one);
        lc4_draw_sprite(8, 0, YELLOW, eight);
    }
    if (snake_length == 19) {
        lc4_draw_sprite(0, 0, YELLOW, one);
        lc4_draw_sprite(8, 0, YELLOW, nine);
    }
    if (snake_length == 20) {
        lc4_draw_sprite(0, 0, YELLOW, two);
        lc4_draw_sprite(8, 0, YELLOW, zero);
...
void play_game() {
    ...
    while (state == GAME_PLAYING) {
        ...
        keyboard_input = lc4_getc_timer(timer); 
